...
XGo is a solid project but targets a different niche. Kukicha's focus is:

- **English keywords** — `reference of user` instead of `&user`, `and`/`or` instead of `&&`/`||`, making generated code reviewable by non-programmers
- **The pipe operator** — built-in data-flow style with placeholder support (`_`)
- **`onerr`** — inline error handling without hiding errors behind exceptions
- **Agent workflow** — syntax designed to be generated by AI and approved by humans
