...
`goStdlibType` carries nested type info for compound types: `ElementType *goStdlibType` for lists, `KeyType`/`ValueType *goStdlibType` for maps. This allows the semantic analyzer to propagate element types through pipe chains (e.g., `string.Lines()` returns `TypeKindList` with `ElementType: TypeKindString`). `goStdlibEntryToTypeInfos` uses `goStdlibTypeToTypeInfo` to recursively convert these to `TypeInfo` trees.
...
`inferLambdaParamTypes` is called in `analyzeCallExpr`; `inferLambdaParamTypesMethod` is called in `analyzeMethodCallExpr`. Both record inferred types in `a.exprTypes` so codegen can emit fully typed Go func literals.

**Analysis ordering:** In both `analyzeCallExpr` and `analyzeMethodCallExpr`, non-lambda arguments are analyzed first, then lambda param types are inferred, then lambda bodies are analyzed. This ensures lambda parameters have their inferred types in the symbol table when the body is analyzed (e.g., `e.name` correctly resolves `name` on `RepoEntry` when `e` is inferred). The `ArrowLambda` case in `analyzeExpression` checks `exprTypes[param.Name]` for pre-inferred types when defining parameters in the scope.

**Generic return type resolution:** After getting return types from the Kukicha stdlib registry, `resolveGenericPlaceholders` resolves placeholder element types using actual call-site arguments. For `any`/`any2`/`ordered` placeholders, it uses the element type of the first list argument. For `result` placeholders, it uses the return type of the first lambda argument (derived from the lambda's `TypeKindFunction` return). The `ArrowLambda` analysis returns `&TypeInfo{Kind: TypeKindFunction, Returns: [bodyType]}` for single-expression lambdas, enabling this resolution.
